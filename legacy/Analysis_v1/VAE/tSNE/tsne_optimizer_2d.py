...
vae = VAE(input_dim=len(features), latent_dim=3).to(device)
vae.load_state_dict(torch.load(latest_vae, map_location=device), strict=False)
vae.eval()

with torch.no_grad():
    latent_64d = vae.encode(X_tensor).cpu().numpy()

# ==========================================
# 4. t-SNE OPTIMIZATION (N=2)
# ==========================================
print("Running t-SNE to reduce VAE manifold from 64D to 2D...")
# Perplexity usually kept between 5 and 50 based on dataset size
tsne = TSNE(n_components=2, perplexity=30, max_iter=1000, random_state=42)
latent_2d = tsne.fit_transform(latent_64d)
...
# Save the embedding
output_file = os.path.join(TSNE_MODEL_DIR, "tsne_embeddings_2d.pkl")
